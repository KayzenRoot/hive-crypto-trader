...
class InMemoryExchangeReferenceAdapter:
    """Deterministic, credential-free and network-free adapter for tests."""
    ...
    def resolve_reference(
        ...
        *,
        ...
        if contract_id is not None:
            _safe_identity(contract_id, IdentityKind.INSTRUMENT, "contract")
        if native_symbol is not None and not _SYMBOL_PATTERN.fullmatch(native_symbol):
            raise MalformedReferenceError("invalid native symbol")
        matches = [
            reference
